...
// Auto-generate screenshots based on app name + category
const CATEGORY_MOCKUPS = {
  Productivity: ['editor', 'form', 'board', 'list'],
  Social:       ['chat', 'feed', 'cards', 'list'],
  Games:        ['game', 'grid', 'chart', 'board'],
  Finance:      ['chart', 'gauge', 'list', 'form'],
  Health:       ['gauge', 'chart', 'list', 'cards'],
  Education:    ['editor', 'list', 'cards', 'grid'],
  Entertainment:['player', 'grid', 'feed', 'cards'],
  Shopping:     ['grid', 'cards', 'list', 'form'],
  Travel:       ['map', 'list', 'cards', 'grid'],
  News:         ['feed', 'list', 'cards', 'editor'],
  Music:        ['player', 'list', 'grid', 'feed'],
  Utilities:    ['form', 'list', 'gauge', 'editor'],
}
const SS_COLORS = ['#6c5ce7', '#00b894', '#0984e3', '#e17055']

function generateScreenshots(appName, category) {
  const mockups = CATEGORY_MOCKUPS[category] || ['editor', 'list', 'grid', 'chart']
  const name = appName.trim() || 'App'
  const titles = [`${name} — Main View`, `${name} — Browse`, `${name} — Details`, `${name} — Overview`]
  const captions = ['Main interface', 'Content browsing', 'Detailed view', 'Quick overview']
                ...
                <div style={{display:'flex',gap:12}}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const autoScreenshots = generateScreenshots(appName, category)
                      const newApp = {
